...
dataset_dir= "./Dataset/CN_AD" # 数据集根目录
dataset_dir_test = "./Dataset"
...
def forward_g(inputs, targets):
    ...
    a_value = floor(Tensor(math.log(max_value/a, 10),ms.float32)).astype(np.int32)
    b_value = floor(Tensor(math.log(max_value/b, 10),ms.float32)).astype(np.int32)
    # c_value = floor(math.log(max_value/c, 10)).astype(np.int32)
...
# 前160 epoch 还未引入鉴别器（Phase 1）和状态鉴别器（Phase 2）
